...
bool IsIntersectionInternal(std::shared_ptr<const GraphTile> start_tile,
  ...
  // Get the tile at the startnode
  std::shared_ptr<const GraphTile> tile = start_tile;
...
void GetHeadings(std::shared_ptr<const GraphTileBuilder> tile, NodeInfo& nodeinfo, uint32_t ntrans) {
  if (ntrans == 0) {
...
bool IsNextEdgeInternalImpl(const DirectedEdge directededge,
                            std::shared_ptr<const GraphTileBuilder> tilebuilder,
                            std::shared_ptr<const GraphTileBuilder> end_node_tile,
 ...
 */
uint32_t GetOpposingEdgeIndex(std::shared_ptr<const GraphTile> endnodetile,
                              const GraphId& startnode,
                              std::shared_ptr<const GraphTile> tile,
