...
bool expand_from_node(const sif::mode_costing_t& mode_costing,
  ...
  for (uint32_t i = start_de; i < nodeinfo->edge_count(); i++, de++, ++edge_id) {
    ...
    std::shared_ptr<const GraphTile> end_node_tile = reader.GetGraphTile(de->endnode());
  ...
  if (!from_transition && nodeinfo->transition_count() > 0) {
    ...
    for (uint32_t i = start_trans; i < nodeinfo->transition_count(); ++i, ++trans) {
      ...
      std::shared_ptr<const GraphTile> end_node_tile = reader.GetGraphTile(trans->endnode());
      if (end_node_tile == nullptr) {
        continue;
      }
...
bool RouteMatcher::FormPath(const sif::mode_costing_t& mode_costing,
  ...
  for (const auto& edge : options.locations().begin()->path_edges()) {
    ...
    std::shared_ptr<const GraphTile> begin_edge_tile = reader.GetGraphTile(graphid);
    ...
    while (index < shape.size()) {
      ...
      if (shape.at(index).lnglat().ApproximatelyEqual(de_end_ll) &&
        ...
        if (expand_from_node(mode_costing, mode, reader, shape, distances, origin_epoch,
          ...
          std::shared_ptr<const GraphTile> end_edge_tile = reader.GetGraphTile(end_edge_graphid);
